...
const Message = ({ msg, myId }) => {
  ...
  const getTick = () => {
    ...
    if (msg.status === "delivered" || msg.status === "seen") {
      return (
        <span
          className={`relative inline-flex h-4 w-5 ${
        ...
        >
          ...
          <svg
            className="absolute left-1.5 top-0 h-4 w-4"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
    ...
    <>
      <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
        ...
        >
          <div
            ref={menuRef}
            className={`relative w-full rounded-[26px] ${msg.isDeletedForEveryone ? "px-3 py-2" : "px-4 py-3"} ${bubbleClasses}`}
            onClick={() => {
          ...
          >
            {showMenu && !editing && !msg.isDeletedForEveryone && (
              <div className="absolute bottom-full right-0 z-100 mb-2 w-40 rounded-2xl border border-slate-200 bg-white p-1 text-sm text-slate-700 shadow-xl">
                {isMe && (
                  <button
                        ...
                        >
                          ...
                          {kind === "video" && (
                            <div className="relative flex h-auto max-h-72 w-full items-center justify-center bg-black">
                              ...
                              />
                              <div className="absolute inset-0 flex items-center justify-center text-3xl text-white opacity-80 drop-shadow-md">
                                ▶
                              </div>
                            </div>
                          )}
